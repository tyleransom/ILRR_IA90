...
set more off
capture log close
log using effectsIA90analysisISplacebo.log, replace
...
gen     forshr_16pl_stemo_00_90_2559_p86 = forshr_16pl_stemo_00_90_2559*post86
...
preserve
    ...
    collapse stem_maj stemocc unemployed nilf lincearn [aweight=perwt] if inrange(yearage18,1981,1989), by(bpl yearage18 ed16plus female black)
...
preserve
    ...
    replace unemployed=1 if empstat==2
    replace nilf=1       if empstat==3
    collapse stem_maj stemocc unemployed nilf lincearn [aweight=perwt] if inrange(yearage18,1981,1989), by(bpl yearage18 female black)
...
preserve
    ...
    replace unemployed=1 if empstat==2
    replace nilf=1       if empstat==3
    collapse stemocc unemployed nilf lincearn [aweight=perwt] if inrange(yearage18,1981,1989), by(bpl yearage18 ed16plus stem_maj female black)
...
qui sum stem_maj  [aweight=perwt] if ed16plus==1 & female==0 & black==1 & inrange(yearage18,1981,1984) & ~inlist(bpl,13,5,38,29)
local mu1A  = `r(mean)'
qui sum emp_stemocc  [aweight=perwt] if ed16plus==1 & stem_maj==1 & female==0 & black==0 & inrange(yearage18,1981,1984) & ~inlist(bpl,13,5,38,29)
local mu2A  = `r(mean)'
qui sum workedly  [aweight=perwt] if ed16plus==1 & stem_maj==1 & female==1 & black==0 & inrange(yearage18,1981,1984) & ~inlist(bpl,13,5,38,29)
local mu3A  = `r(mean)'

*Table: Main Results Using Base Specification
qui reg stem_maj forshr_16pl_stemo_80_2559_p86 i.bpl##c.yearage18  lnpop18 unemprate_styr lnmedhhinc18 yeardum* agedum* yearbirthdum*   [aweight=perwt] if female==0 &  black==1    & inrange(yearage18,1981,1989) & ~inlist(yearage18,1985) & ~inlist(bpl,13,5,38,29) & ed16plus==1 , cluster(bpl)
first_coef, in1(`e(cmdline)') in2(`e(N)') out(b1A se1A star1A N1A)
est sto regA
* outreg using ${outreg_path}IStableS0.doc, replace se starlevels(10 5 1) starloc(1)
qui reg emp_stemocc  forshr_16pl_stemo_80_2559_p86 i.bpl##c.yearage18  lnpop18 unemprate_styr lnmedhhinc18 yeardum* agedum* yearbirthdum*   [aweight=perwt] if female==0 &  white==1    & inrange(yearage18,1981,1989) & ~inlist(yearage18,1985) & ~inlist(bpl,13,5,38,29) & ed16plus==1 & stem_maj==1, cluster(bpl)
...
qui reg workedly forshr_16pl_stemo_80_2559_p86 i.bpl##c.yearage18  lnpop18 unemprate_styr lnmedhhinc18 yeardum* agedum* yearbirthdum*   [aweight=perwt] if female==1 &  white==1    & inrange(yearage18,1981,1989) & ~inlist(yearage18,1985) & ~inlist(bpl,13,5,38,29) & ed16plus==1 & stem_maj==1, cluster(bpl)
first_coef, in1(`e(cmdline)') in2(`e(N)') out(b1C se1C star1C N1C)
est sto regC
* outreg using ${outreg_path}IStableS0.doc, append   se starlevels(10 5 1) starloc(1)
est table regA regB regC , b(%9.3f) se(%9.3f) stats(N) keep(forshr_16pl_stemo_80_2559_p86) title("Table S00: Re-print main results on black male STEM major, white male STEM occupation, white female employment")
...
file write TA00 "Foreign STEM Exposure" " & " %4.3f (`b1A') "`star1A'" " & " %4.3f (`b1B') "`star1B'" " & " %4.3f (`b1C') "`star1C'" " \\ "  _n 
file write TA00                         " &  (" %4.3f (`se1A') ")        & (" %4.3f  (`se1B') ")        & (" %4.3f  (`se1C') ") \\ "  _n 
